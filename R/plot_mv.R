...
#' @param design Design matrix with rows corresponding to samples and columns to
#'   coefficients to be estimated. Only relevant for
...
#' TRUE}), \link[DESeq2]{vst}, \link[DESeq2]{rlog}}, etc. If \code{dat} is of class
#' \code{\link[limma]{MArrayLM}} or \code{\link[edgeR]{DGEGLM}}, then the y-axis
#' represents residual variance. For \code{\link[DESeq2]{DESeqDataSet}}s, the choice
#' of whether to plot variance or residual variance is determined by the \code{resid}
#' argument. For data that has been fit with a negative binomial model, a residual
...
#' This is because the \vode{voom} transform involves an initial modeling step in which
...
plot_mv.MArrayLM <- function(dat,
  ...
  p <- ggplot(df, text = Probe) +
    geom_smooth(aes(Mu_lo, Sigma_lo, color = 'LOWESS'),
...
plot_mv.DGEList <- function(dat,
  ...
  if (all(dat$samples$norm.factors == 1L)) {     # Calculate size factors for lcpm
    dat <- calcNormFactors(dat)
  }
  if (is.null(dat$tagwise.dispersion)) {         # Calculate tagwise dispersions for aveLogCPM
    if (is.null(dat$design) & is.null(design)) {
      dat <- estimateTagwiseDisp(dat)
      warning('No design matrix detected or supplied. Genewise dispersion estimates ',
              'will be calculated using the method of Robinson & Smyth (2007). See ',
              '?estimateTagwiseDisp.')
    } else {
      if (!is.null(design)) {
        dat$design <- design
      }
      dat <- estimateGLMTagwiseDisp(dat)
    }
  }
  ...
  p <- ggplot(df, aes(text = Probe)) +
    ...
    geom_smooth(aes(Mu_lo, Sigma_lo, color = 'LOWESS'), size = 0.5, se = FALSE,
                method = 'gam', formula = y ~ s(x, bs = 'cs')) +
