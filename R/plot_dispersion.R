...
#'   \code{edgeR} or \code{DESeq2} functions. Alternatively, \code{dat} may be a raw
#'   count matrix, in which case the \code{pipeline} argument must specify which
#'   software to use for estimating normalization factors and genewise dispersions.
#'   A design matrix may be required to calculate adjusted profile log-likelihoods.
#'   See Details.
#' @param design Optional design matrix with rows corresponding to samples and columns
#'   to coefficients to be estimated. This will be extracted from \code{dat}
...
#' @param pipeline Which package should be used to estimate normalization factors
#'   and genewise dispersions? Only relevant if \code{dat} is a raw count matrix.
#'   Must be one of \code{"edgeR"} or \code{"DESeq2"}. Default settings are applied
#'   at all steps. For greater control of internal parameters, create the appropriate
#'   \code{DGEList} or \code{DESeqDataSet} object with your preferred settings and
#'   pass it directly as \code{dat}.
...
#'   display or appear in the RStudio viewer. Gene names are extracted from \code{dat}.
#'
#' @details
...
#' dispersions is therefore an essential step in the model fitting process. Becaeuse
#' there are rarely sufficient replicates to reliably infer these values independently
#' for each gene, both packages use empirical Bayes methods to pool information across
#' genes.
#'
#' Details vary between the two pipelines, which is why output will differ for \code{
...
#' statistical background, see the original DESeq paper (Anders & Huber, 2010) and
#' the DESeq2 paper (Love et al., 2014).
#'
...
plot_dispersion.DESeqDataSet <- function(dat,
  ...
  df <- data_frame(Gene = rownames(dat),
                   Mean = aveLogCPM(counts(dat, normalized = TRUE), prior.count = 1L,
...
plot_dispersion.default <- function(dat,
  ...
  if (pipeline == 'edgeR') {
    ...
    plot_dispersion(dat, trans = trans, title = title, legend = legend, hover = hover)
  } else {
